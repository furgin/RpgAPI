...
public class BasicChatBehavior implements ChatBehavior{
    ...
    public void chatChannel(String chatName, String receiveName, String sendName, String language, String message, Boolean chatSpy){
        ...
        if (chatSpy && rp.isSpyingOnChats()){
                Bukkit.getPlayer(receiveName).sendMessage(ChatProcessor.TitleString(RpgAPI.nameDisplays, sendName, receiveName) + ChatColors.ChatString(message));
                return;
        }
        ...
        if (!(languageKnown)){
            ...
            Bukkit.getPlayer(receiveName).sendMessage(ChatProcessor.TitleString(RpgAPI.nameDisplays, sendName, receiveName) + ChatColors.ChatString(LanguageProcessor.LanguageDecoder(message, language)));
            return;
            } else {
                return;
            }
